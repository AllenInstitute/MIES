...
if [[ -n "$matches" ]]
then
  echo "The hardware tests should never manually lock a device. Prefer ACD_AcquireData(...) or ACD_CreateLockedDAEphys(...)."
  echo "$matches"
  ret=1
fi
...
matches=$(git grep $opts 'JSON_Exists(.*\"\")' '**/MIES_*.ipf' '**/UTF*.ipf')

if [[ -n "$matches" ]]
then
  echo "Prefer JSON_IsValid(...) over JSON_Exists(..., \"\")"
  echo "$matches"
  ret=1
fi

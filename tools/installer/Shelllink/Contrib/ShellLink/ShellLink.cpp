...
void ShortCutData(int nType)
{
  ...
  TCHAR* szBuf2 = (TCHAR*)LocalAlloc(LPTR, sizeof(TCHAR)*MAX_PATH);
	...
	if (hRes == S_OK)
	{
		...
		if (hRes == S_OK)
		{
			fileMode = (nType <= SHELLLINKTYPE_GETWORKINGDIR) ? STGM_READ : STGM_READWRITE;

#ifdef UNICODE
			hRes=ppf->Load(szBuf, fileMode);
#else
      WCHAR* wszPath = (WCHAR*)LocalAlloc(LPTR, sizeof(WCHAR)*MAX_PATH);
			MultiByteToWideChar(CP_ACP, 0, szBuf, -1, wszPath, MAX_PATH);
			hRes=ppf->Load(wszPath, fileMode);
      LocalFree(wszPath);
#endif
			if (hRes == S_OK)
			{
				if (nType <= SHELLLINKTYPE_GETWORKINGDIR)
				{
					...
					switch(nType)
					{
						...
						case SHELLLINKTYPE_GETPATH:
						{
							...
							hRes=psl->GetPath(szBuf, MAX_PATH, &fd, NULL);
							if (hRes != S_OK) szBuf[0] = '\0';
							#ifdef SHELLLINK_DEBUG
							  MessageBox(hwndParent, TEXT("GETPATH Result"), TEXT(szBuf), MB_OK);
							#endif
						}; break;

...
*/
	...
	HOST_EXPORT void setmenuitemtext(MenuHandle menuH, short item, const char* itemString);
	HOST_EXPORT void DeleteMenuItem(MenuHandle theMenu, short item);
	HOST_EXPORT void insertmenuitem(MenuHandle menuH, const char *itemString, short afterItemOneBased);
	HOST_EXPORT void appendmenu(MenuHandle menuH, const char *itemString);
	...
	HOST_EXPORT int PtrToHand(const void* srcPtr, Handle* destHandlePtr, BCInt size);
	HOST_EXPORT int PtrAndHand(const void* p, Handle h, BCInt size);
	...
	HOST_EXPORT int SetScrapData(void* theWindow, int length, UInt32 theType, const void *source);

...
static Function TestOperationPreparefit()
	...
	WAVE/WAVE output  = SFE_ExecuteFormula(str, win, useVariables = 0)
	WAVE/T    fitArgs = output[%FITARGS]
	CHECK_EQUAL_STR(fitArgs[%HOLDSTR], "000")
...
static Function TestOperationFit2_ODR()
	...
	WAVE yErrorsMinus = JWN_GetNumericWaveFromWaveNote(output, SF_META_ERRORBARYMINUS)
	CHECK_WAVE(yErrorsMinus, NUMERIC_WAVE)
	CHECK_EQUAL_VAR(DimSize(yErrorsMinus, ROWS), 3)
	CHECK_CLOSE_VAR(yErrorsMinus[0], 0.4356034023045619, tol = 5E-15)
	WAVE yErrorsPlus = JWN_GetNumericWaveFromWaveNote(output, SF_META_ERRORBARYPLUS)
	CHECK_WAVE(yErrorsPlus, NUMERIC_WAVE)
	CHECK_EQUAL_VAR(DimSize(yErrorsPlus, ROWS), 3)
	CHECK_CLOSE_VAR(yErrorsPlus[0], 0.4356034023045619, tol = 5E-15)

	WAVE xErrorsMinus = JWN_GetNumericWaveFromWaveNote(output, SF_META_ERRORBARXMINUS)
	CHECK_WAVE(xErrorsMinus, NUMERIC_WAVE)
	CHECK_EQUAL_VAR(DimSize(xErrorsMinus, ROWS), 3)
	CHECK_CLOSE_VAR(xErrorsMinus[0], 0.1257036566393522, tol = 5E-15)
	WAVE xErrorsPlus = JWN_GetNumericWaveFromWaveNote(output, SF_META_ERRORBARXPLUS)
	CHECK_WAVE(xErrorsPlus, NUMERIC_WAVE)
	CHECK_EQUAL_VAR(DimSize(xErrorsPlus, ROWS), 3)
	CHECK_CLOSE_VAR(xErrorsPlus[0], 0.1257036566393522, tol = 5E-15)

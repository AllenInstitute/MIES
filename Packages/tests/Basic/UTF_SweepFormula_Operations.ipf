...
static Function TestOperationLabNotebook()
	...
	str = "labnotebook(" + textKey + ", select(channels(AD2), [0, 1]))"
	WAVE/WAVE dataRef = SF_ExecuteFormula(str, win, useVariables = 0)
	...
	// multiple keys
	str = "labnotebook([\"ADC\", \"Operating Mode\"], select(channels(AD2), [0]), DATA_ACQUISITION_MODE)"
	WAVE/WAVE dataRef = SF_ExecuteFormula(str, win, useVariables = 0)
	...
	str = "labnotebook([\"eee*\"], select(channels(AD2), [0]), DATA_ACQUISITION_MODE)"
	WAVE/WAVE dataRef = SF_ExecuteFormula(str, win, useVariables = 0)
	CHECK_EQUAL_VAR(DimSize(dataRef, ROWS), 0)

	// match with wildcard and special QC format
	str = "labnotebook([\"*random QC\"], select(channels(AD0), [0, 1, 2]), DATA_ACQUISITION_MODE)"
	WAVE/WAVE dataRef = SF_ExecuteFormula(str, win, useVariables = 0)
	...
	// key with unit
	str = "labnotebook([\"*other KEY\"], select(channels(AD0), [0]), DATA_ACQUISITION_MODE)"
	WAVE/WAVE dataRef = SF_ExecuteFormula(str, win, useVariables = 0)
	CHECK_EQUAL_VAR(DimSize(dataRef, ROWS), 1)
...
static Function TestOperationAnaFuncParam()
	...
	// no params in sweep
	str = "anaFuncParam(\"var\", select(channels(), [2]))"
	WAVE/WAVE dataRef = SF_ExecuteFormula(str, win, useVariables = 0)
	CHECK_EQUAL_VAR(DimSize(dataRef, ROWS), 0)
	...
	// no select data
	str = "anaFuncParam(\"var\", select(channels(), [100]))"
	WAVE/WAVE dataRef = SF_ExecuteFormula(str, win, useVariables = 0)
	CHECK_EQUAL_VAR(DimSize(dataRef, ROWS), 0)

	// single match (var)
	str = "anaFuncParam(\"var\", select(channels(AD0), sweeps()))"
	WAVE/WAVE dataRef = SF_ExecuteFormula(str, win, useVariables = 0)
	...
	// wildcard match (str*)
	str = "anaFuncParam(\"str*\", select(channels(AD0), [0, 1]))"
	WAVE/WAVE dataRef = SF_ExecuteFormula(str, win, useVariables = 0)
	CHECK_EQUAL_VAR(DimSize(dataRef, ROWS), 4)
	idx = 0
	...
	// wildcard match (wv*)
	str = "anaFuncParam(\"wv*\", select(channels(AD0), [0, 1]))"
	WAVE/WAVE dataRef = SF_ExecuteFormula(str, win, useVariables = 0)
	CHECK_EQUAL_VAR(DimSize(dataRef, ROWS), 4)
	idx = 0
	Make/FREE dataPoints = {0, NaN, 0, NaN}

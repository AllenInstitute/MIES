...
static Function EnsureUnityGain(string device, variable headstage)
	...
	ret = AI_WriteToAmplifier(device, headstage, mode, MCC_SETPRIMARYSIGNALGAIN_FUNC, 1)
	CHECK(!ret)

	ret = AI_WriteToAmplifier(device, headstage, mode, MCC_SETSECONDARYSIGNALGAIN_FUNC, 1)
	CHECK(!ret)

	gain = AI_ReadFromAmplifier(device, headstage, mode, MCC_GETPRIMARYSIGNALGAIN_FUNC, NaN)
	REQUIRE_EQUAL_VAR(gain, 1)

	gain = AI_ReadFromAmplifier(device, headstage, mode, MCC_GETSECONDARYSIGNALGAIN_FUNC, NaN)
	REQUIRE_EQUAL_VAR(gain, 1)
End

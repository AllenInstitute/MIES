...
Function/S GetTestCode(string postProc, [string eventState, string prop])
	...
	code  = "psx(myId, psxKernel(select(selrange([50, 150]), selchannels(AD6), selsweeps([0, 2]), selvis(all))), 0.3, psxSweepBPFilter(100, 0))"
	code += "\r and \r"
...
static Function/S GetDefaultTestSuitesForExperiment()
	...
	strswitch(match)
		case "UTF_Basic.ipf":
			...
			list = AddListItem("UTF_Downsample.ipf", list, ";", Inf)
			...
			list = AddListItem("UTF_Hashmap.ipf", list, ";", Inf)
			list = AddListItem("UTF_InputDialog.ipf", list, ";", Inf)
...
Function RunWithOpts([string testcase, string testsuite, variable allowDebug, variable instru, string traceWinList, variable ITCXOP2Debug, variable keepDataFolder, variable enableJU, variable enableRegExp])
	...
	if(IsEmpty(testcase))
		RunTest(testsuite, name = name, enableJU = enableJU, enableRegExp = enableRegExp, debugMode = debugMode, traceOptions = traceOptions, traceWinList = traceWinList, keepDataFolder = keepDataFolder, waveTrackingMode = waveTrackingMode, retry = IUTF_RETRY_FAILED_UNTIL_PASS)
	else

...
The full plotting specification is a wave reference wave tree with two levels. The top most level indexes over evaluated formula results as if
...
The evaluation call to `SFH_ExecuteFormulaInternal` uses the current variable storage. This allows to provide already evaluated variables.

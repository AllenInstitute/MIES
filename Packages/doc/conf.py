...
def get_version():
    ...
    branchString = (
        Popen("git rev-parse --abbrev-ref HEAD", stdout=PIPE, shell=True)
        .stdout.read()
        .rstrip()
    )
    revString = (
        Popen(
            'git describe --always --tags --match "Release_*"', stdout=PIPE, shell=True
        )
        .stdout.read()
        .rstrip()
    )

    return "({branch}) {version}".format(
        branch=branchString.decode("ascii"), version=revString.decode("ascii")
    )

...
static Constant RD_MAX_POINT_DIFFERENCE_SWEEP_DATA = 10

static Function RD_CloseEverything()
...
/// @brief Return the used device for single device experiments or NONE otherwise
static Function/S RD_GetUsedSingleDevice()

	string   matches
	variable settingsCol

	DFREF dfr = GetReplayStorage()

	matches = GetListOfObjects(dfr, LBN_TEXTUAL_VALUES_NAME, fullPath = 1, recursive = 1, exprType = MATCH_WILDCARD)

	if(ItemsInList(matches) > 1)
		return NONE
	endif

	WAVE/T textualValues_replay = $StringFromList(0, matches)

	[WAVE indizes, settingsCol] = GetNonEmptyLBNRows(textualValues_replay, "Device")

	if(!WaveExists(indizes))
		return NONE
	endif

	Make/FREE/T/N=(DimSize(indizes, ROWS), LABNOTEBOOK_LAYER_COUNT) devices = textualValues_replay[indizes[p]][settingsCol][q]
	Redimension/N=(numpnts(devices))/E=1 devices

	WAVE/T uniqueDevices = GetUniqueEntries(devices, caseSensitive = 0)

	RemoveTextWaveEntry1D(uniqueDevices, "")

	if(DimSize(uniqueDevices, ROWS) == 1)
		return uniqueDevices[0]
	endif

	return NONE
End

static Function RD_GetUsedSingleHeadstage(string device, variable sweepNo)
	...
	if(Sum(statusHS, 0, NUM_HEADSTAGES - 1) != 1)
		return NaN
	endif

	return GetRowIndex(statusHS, val = 1)
End
...
static Function RD_CompareSweepData(string device, variable sweepNo)
	...
	string name
	variable i, diffCount, numRows, pointDiffCount
	...
	for(name : wvs)
		...
		for(i = 0; i < numRows && pointDiffCount < RD_MAX_POINT_DIFFERENCE_SWEEP_DATA; i += 1)
...
static Function RD_ApplySettingsFromPreviousSweep(string device, variable sweepNo)
	...
	WAVE/Z s.abm = GetLastSetting(numericalValues, sweepNo, "Autobias Ibias max", DATA_ACQUISITION_MODE)
...
static Function RD_ExtractSingleValue(WAVE/Z wv)
...
static Function RD_CompareSweeps(WAVE sweeps, string device)

	variable sweepNo, diffCount
	...
	for(sweepNo : sweeps)
		...
		diffCount += RD_CompareSweepData(device, sweepNo)
		diffCount += RD_CompareLabnotebooks(device = device, sweepNo = sweepNo)
	endfor
...
/// for the current sweep or the next sweep.
///
/// @param device        device
...
///        device and sweep
Function RD_CompareLabnotebooks([variable sweepNo, string device])

	variable diffCount

	if(ParamIsDefault(sweepNo))
		sweepNo = ROVar(GetReplayRefSweep())
	endif

	if(ParamIsDefault(device))
		device = ROStr(GetReplayRefDevice())
	endif
	...
	diffCount += RD_CompareLabnotebooks_Impl(numericalValues_new, numericalValues_replay, sweepNo)
	diffCount += RD_CompareLabnotebooks_Impl(textualValues_new, textualValues_replay, sweepNo)
...
Function RD_CompareSCI([string device, variable sweepNo, variable headstage])
	...
	if(ParamIsDefault(headstage))
		headstage = RD_GetUsedSingleHeadstage(device, sweepNo)
		ASSERT(IsValidHeadstage(headstage), "Missing optional argument headstage as multiple headstages were active.")
	endif
	...
	return RD_CompareSweeps(sweeps, device)
End

/// @brief Compare the labnotebook entries and sweep data for `new` and `replay` of
///        the given device and all sweeps of the repeated acquisition cycle of `sweepNo`
Function RD_CompareRAC([string device, variable sweepNo])
...
/// @brief Transfer data back from sweep data into DAQDataWave
Function RD_UpdateData(string device, WAVE allGain, WAVE dataWave, variable fifoPosGlobal, variable fifoPos)

	variable sweepNo, numRows, chan, i, fifoEndWritePos, clearBefore
...
/// @brief Prepare the current experiment for replaying data
...
/// @brief Replay the already acquired data of `device` starting with `refSweepNo`
Function RD_ReplayData(variable refSweepNo, [string device])
	...
	if(ParamIsDefault(device))
		device = RD_GetUsedSingleDevice()

		ASSERT(cmpstr(device, NONE), "Optional argument device was not passed but multiple devices were in use, please pass it.")
	endif

	SVAR refDevice = $GetReplayRefDevice()
	refDevice = device
	...
	// remove hashmaps due to branch switching messing up the version upgrades as downgrades are not supported
	WAVE numericalValues_new = GetLBnumericalValues(device)
	KillOrMoveToTrash(wv = GetLogbookKeyHashmap(numericalValues_new))

	WAVE numericalValues_replay = $RD_TranslateToReplay(wv = numericalValues_new)
	KillOrMoveToTrash(wv = GetLogbookKeyHashmap(numericalValues_replay))

	WAVE/T textualValues_new = GetLBtextualValues(device)
	KillOrMoveToTrash(wv = GetLogbookKeyHashmap(textualValues_new))

	WAVE/T textualValues_replay = $RD_TranslateToReplay(wv = textualValues_new)
	KillOrMoveToTrash(wv = GetLogbookKeyHashmap(textualValues_replay))
...
Function RD_IsEnabled()

	return ROvar(GetReplayDataEnable())
End

/// @brief Return the truth that replaying data compilation support is enabled
Function RD_IsCompilationEnabled()

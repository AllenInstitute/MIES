...
Function IVS_ConfigureMCC()
	...
	if(AI_SelectMultiClamp(device, headstage) != AMPLIFIER_CONNECTION_SUCCESS)
		...
		initResult = AI_WriteToAmplifier(device, headstage, I_CLAMP_MODE, MCC_SETBRIDGEBALENABLE_FUNC, 0)
		if(!initResult)
			print "Error setting Bridge Balance Enable to off"
			numErrors += 1
		endif

		initResult = AI_WriteToAmplifier(device, headstage, I_CLAMP_MODE, MCC_SETNEUTRALIZATIONCAP_FUNC, 0.0)
		if(!initResult)
			print "Error setting Neutralization Cap to 0.0"
			numErrors += 1
		endif

		initResult = AI_WriteToAmplifier(device, headstage, I_CLAMP_MODE, MCC_SETNEUTRALIZATIONENABL_FUNC, 0)
		if(!initResult)
			print "Error setting Neutralization Enable to off"
			numErrors += 1
		endif

		initResult = AI_WriteToAmplifier(device, headstage, I_CLAMP_MODE, MCC_SETSLOWCURRENTINJENABL_FUNC, 0)
		if(!initResult)
			print "Error setting  SlowCurrentInjEnable to off"
			numErrors += 1
		endif

		initResult = AI_WriteToAmplifier(device, headstage, I_CLAMP_MODE, MCC_SETSLOWCURRENTINJLEVEL_FUNC, 0.0)
		if(!initResult)
			print "Error setting SlowCurrentInjLevel to 0"
			numErrors += 1
		endif

		initResult = AI_WriteToAmplifier(device, headstage, I_CLAMP_MODE, MCC_SETSLOWCURRENTINJSETLT_FUNC, 1)
		if(!initResult)
		...
		initResult = AI_WriteToAmplifier(device, headstage, I_CLAMP_MODE, MCC_SETHOLDING_FUNC, 0.0)
		if(!initResult)
			print "Error setting Holding Voltage to 0.0"
			numErrors += 1
		endif

		initResult = AI_WriteToAmplifier(device, headstage, I_CLAMP_MODE, MCC_SETHOLDINGENABLE_FUNC, 0)
		if(!initResult)
			print "Error setting Holding Enable to off"
			numErrors += 1
		endif

		initResult = AI_WriteToAmplifier(device, headstage, I_CLAMP_MODE, MCC_SETOSCKILLERENABLE_FUNC, 1)
		if(!initResult)
		...
		initResult = AI_WriteToAmplifier(device, headstage, V_CLAMP_MODE, MCC_SETHOLDING_FUNC, 0.0)
		if(!initResult)
			print "Error setting Holding Voltage to 0.0"
			numErrors += 1
		endif

		initResult = AI_WriteToAmplifier(device, headstage, V_CLAMP_MODE, MCC_SETHOLDINGENABLE_FUNC, 0)
		if(!initResult)
			print "Error setting Holding Enable to off"
			numErrors += 1
		endif

		initResult = AI_WriteToAmplifier(device, headstage, V_CLAMP_MODE, MCC_SETOSCKILLERENABLE_FUNC, 1)
		if(!initResult)
			print "Error setting OscKillerEnable to on"
			numErrors += 1
		endif
		...
		initResult = AI_WriteToAmplifier(device, headstage, V_CLAMP_MODE, MCC_SETRSCOMPCORRECTION_FUNC, 0.0)
		if(!initResult)
			print "Error setting RsCompCorrection to 0"
			numErrors += 1
		endif

		initResult = AI_WriteToAmplifier(device, headstage, V_CLAMP_MODE, MCC_SETRSCOMPENABLE_FUNC, 0)
		if(!initResult)
			print "Error setting RsCompEnable to off"
			numErrors += 1
		endif

		initResult = AI_WriteToAmplifier(device, headstage, V_CLAMP_MODE, MCC_SETRSCOMPPREDICTION_FUNC, 0)
		if(!initResult)
			print "Error setting RsCompPrediction to off"
			numErrors += 1
		endif

		initResult = AI_WriteToAmplifier(device, headstage, V_CLAMP_MODE, MCC_SETSLOWCOMPTAUX20ENAB_FUNC, 0)
		if(!initResult)
			print "Error setting SlowCompTauX20Enable to off"
			numErrors += 1
		endif

		initResult = AI_WriteToAmplifier(device, headstage, V_CLAMP_MODE, MCC_SETRSCOMPBANDWIDTH_FUNC, 0.0)
		if(!initResult)
			print "Error setting RsCompBandwidth to 0"
			numErrors += 1
		endif

		initResult = AI_WriteToAmplifier(device, headstage, V_CLAMP_MODE, MCC_SETWHOLECELLCOMPCAP_FUNC, 0.0)
		if(!initResult)
			print "Error setting WholeCellCompCap to 0"
			numErrors += 1
		endif

		initResult = AI_WriteToAmplifier(device, headstage, V_CLAMP_MODE, MCC_SETWHOLECELLCOMPENABLE_FUNC, 0)
		if(!initResult)
			print "Error setting  WholeCellCompEnable to off"
			numErrors += 1
		endif

		initResult = AI_WriteToAmplifier(device, headstage, V_CLAMP_MODE, MCC_SETWHOLECELLCOMPRESIST_FUNC, 0)
		if(!initResult)

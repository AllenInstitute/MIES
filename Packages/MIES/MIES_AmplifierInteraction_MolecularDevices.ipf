...
static Function AIMCC_InitAxonTelegraphStruct(tds)
...
/// @param func      MCC function,  one of @ref AIMCC_SendToAmpConstants
static Function AIMCC_GetMCCScale(variable clampMode, variable func)

	if(IsFinite(clampMode))
		AIMCC_AssertOnInvalidClampMode(clampMode)
	endif
...
/// @param value            value to set. values is in MIES units, see AIMCC_SendToAmp()
///                         and there the description of `usePrefixes`.
/// @param sendToAll        should the value be send
///                         to all active headstages (true) or just to the given one (false)
/// @param checkBeforeWrite (ignored for getter functions)
///                         check the current value and do nothing if it is equal within some tolerance to the one written
/// @param selectAmp        Select the amplifier
///                         before use, some callers might save time in doing that once themselves.
/// @param updateView       When set, the amplifier view is updated after setting a value
///
/// @return 0 on success, 1 otherwise
Function AIMCC_UpdateAmpModel(string device, string ctrl, variable headStage, variable value, variable sendToAll, variable checkBeforeWrite, variable selectAmp, variable updateView)

	variable i, diff, clampMode, oppositeMode, oldTab
	...
	for(i = 0; i < NUM_HEADSTAGES; i += 1)
		...
		strswitch(ctrl)
			...
			case "button_DataAcq_WCAuto":
				...
				AIMCC_UpdateAmpView(device, i, ctrl = "check_DatAcq_WholeCellEnable")
				break
			...
			case "setvar_DataAcq_RsCorr":
				...
				if(AmpStorageWave[%RSCompChaining][0][i] && !CheckIfValueIsInsideLimits(device, "setvar_DataAcq_RsPred", AmpStorageWave[%Prediction][0][i] + diff))
					AIMCC_UpdateAmpView(device, i, ctrl = ctrl)
					return 1
				endif
				...
				if(AmpStorageWave[%RSCompChaining][0][i])
					...
					AIMCC_UpdateAmpView(device, i, ctrl = "setvar_DataAcq_RsPred")
				endif
				break
			case "setvar_DataAcq_RsPred":
				...
				if(AmpStorageWave[%RSCompChaining][0][i])
					...
					AIMCC_UpdateAmpView(device, i, ctrl = "setvar_DataAcq_RsCorr")
				endif
				break
			...
			case "button_DataAcq_AutoPipOffset_VC":
				...
				AIMCC_UpdateAmpView(device, i, ctrl = ctrlToCall)
				...
				try
					...
					AIMCC_UpdateAmpView(device, i, ctrl = ctrlToCallOpposite)
			...
			case "check_DataAcq_Amp_Chain":
				...
				AIMCC_UpdateAmpModel(device, "setvar_DataAcq_RsCorr", i, AmpStorageWave[5][0][i], sendToAll, checkBeforeWrite, 0, 1)
				break
			// I-Clamp controls
			...
			case "button_DataAcq_AutoBridgeBal_IC":
				...
				AIMCC_UpdateAmpModel(device, "setvar_DataAcq_BB", i, value, sendToAll, checkBeforeWrite, 0, 1)
				AIMCC_UpdateAmpModel(device, "check_DatAcq_BBEnable", i, 1, sendToAll, checkBeforeWrite, 0, 1)
				break
...
///
/// Disallows setting single controls for outside callers as #AIMCC_UpdateAmpModel should be used for that.
Function AIMCC_SyncAmpStorageToGUI(string device, variable headstage)

	return AIMCC_UpdateAmpView(device, headstage)
End

/// @brief Sync the settings from the GUI to the amp storage wave and the MCC application
Function AIMCC_SyncGUIToAmpStorageAndMCCApp(string device, variable headStage, variable clampMode)
	...
	if(AIMCC_EnsureCorrectMode(device, headStage, 1))
		return NaN
	endif
...
static Function AIMCC_UpdateAmpView(string device, variable headStage, [string ctrl])
	...
	for(i = 0; i < numEntries; i += 1)
		ctrl = StringFromList(i, list)
		lbl  = AIMCC_AmpStorageControlToRowLabel(ctrl)
...
static Function/S AIMCC_AmpStorageControlToRowLabel(string ctrl)
...
Function AIMCC_ZeroAmps(string device, variable headStage)
		...
		if(IsNaN(headstage))
			for(i = 0; i < NUM_HEADSTAGES; i += 1)
				if(abs(TPResults[%BaselineSteadyState][i]) >= ZERO_TOLERANCE)
					AIMCC_MIESAutoPipetteOffset(device, i)
				endif
			endfor
		else
			if(abs(TPResults[%BaselineSteadyState][headstage]) >= ZERO_TOLERANCE)
				AIMCC_MIESAutoPipetteOffset(device, headStage)
			endif
		endif
	endif
End
...
/// @param device device
/// @param headStage MIES headstage number, must be in the range [0, NUM_HEADSTAGES]
static Function AIMCC_MIESAutoPipetteOffset(string device, variable headStage)
...
Function AIMCC_AssertOnInvalidClampMode(variable clampMode)

	ASSERT(AIMCC_IsValidClampMode(clampMode), "invalid clamp mode")
End

/// @brief Return true if the given clamp mode is valid
Function AIMCC_IsValidClampMode(variable clampMode)
...
Function AIMCC_GetHoldingCommand(string device, variable headstage)
	...
	return MCC_GetHoldingEnable() ? MCC_GetHolding() * AIMCC_GetMCCScale(MCC_GetMode(), MCC_GETHOLDING_FUNC) : 0
End
...
static Function AIMCC_SwitchAxonAmpMode()
...
Function AIMCC_SetClampMode(string device, variable headStage, variable mode, variable zeroStep)

	AIMCC_AssertOnInvalidClampMode(mode)
...
/// @param checkBeforeWrite (ignored for getter functions)
///                         check the current value and do nothing if it is equal within some tolerance to the one written
/// @param usePrefixes      Use SI-prefixes common in MIES for the passed and returned values, e.g.
///                         `mV` instead of `V`
/// @param selectAmp        Select the amplifier
///                         before use, some callers might save time in doing that once themselves.
///
/// @returns return value (for getters, respects `usePrefixes`), success (`0`) or error (`NaN`).
Function AIMCC_SendToAmp(string device, variable headStage, variable mode, variable func, variable value, variable checkBeforeWrite, variable usePrefixes, variable selectAmp)

	variable ret, headstageMode, scale
	string str

	scale = usePrefixes ? AIMCC_GetMCCScale(mode, func) : 1
...
/// @paran selectAmp  selects the amplifier
...
Function AIMCC_FillAndSendAmpliferSettings(string device, variable sweepNo)
	...
	for(i = 0; i < NUM_HEADSTAGES; i += 1)
		...
		if(clampMode == V_CLAMP_MODE)
			...
			ampSettingsWave[0][1][i]  = MCC_GetHolding() * AIMCC_GetMCCScale(V_CLAMP_MODE, MCC_GETHOLDING_FUNC)
			ampSettingsWave[0][2][i]  = MCC_GetOscKillerEnable()
			ampSettingsWave[0][3][i]  = MCC_GetRsCompBandwidth() * AIMCC_GetMCCScale(V_CLAMP_MODE, MCC_GETRSCOMPBANDWIDTH_FUNC)
			...
			ampSettingsWave[0][8][i]  = MCC_GetWholeCellCompCap() * AIMCC_GetMCCScale(V_CLAMP_MODE, MCC_GETWHOLECELLCOMPCAP_FUNC)
			ampSettingsWave[0][9][i]  = MCC_GetWholeCellCompResist() * AIMCC_GetMCCScale(V_CLAMP_MODE, MCC_GETWHOLECELLCOMPRESIST_FUNC)
		...
		elseif(clampMode == I_CLAMP_MODE || clampMode == I_EQUAL_ZERO_MODE)
			...
			ampSettingsWave[0][11][i] = MCC_GetHolding() * AIMCC_GetMCCScale(I_CLAMP_MODE, MCC_GETHOLDING_FUNC)
			ampSettingsWave[0][12][i] = MCC_GetNeutralizationEnable()
			ampSettingsWave[0][13][i] = MCC_GetNeutralizationCap() * AIMCC_GetMCCScale(I_CLAMP_MODE, MCC_GETNEUTRALIZATIONCAP_FUNC)
			ampSettingsWave[0][14][i] = MCC_GetBridgeBalEnable()
			ampSettingsWave[0][15][i] = MCC_GetBridgeBalResist() * AIMCC_GetMCCScale(I_CLAMP_MODE, MCC_GETBRIDGEBALRESIST_FUNC)
		...
		// new parameters
		ampSettingsWave[0][35][i] = MCC_GetPipetteOffset() * AIMCC_GetMCCScale(NaN, MCC_GETPIPETTEOFFSET_FUNC)
	endfor
...
Function AIMCC_QueryGainsFromMCC(string device)
	...
	for(i = 0; i < NUM_HEADSTAGES; i += 1)
		...
		if(!clampModeSwitchAllowed)
			printf "AIMCC_QueryGainsFromMCC: Turning off holding potential for automated testing!\r"
...
Function AIMCC_FindConnectedAmps()
	string list

...
Function/WAVE SFO_OperationTP(variable jsonId, string jsonPath, string graph)
	...
	WAVE/Z/WAVE output = SFO_OperationTPIterate(graph, wMode, selectData, ignoreTPs, SF_OP_TP)
	if(!WaveExists(output))
...
static Function SFO_GetTPFitQuality(WAVE residuals, WAVE sweepData, variable beginTrail, variable endTrail)
...
static Function/WAVE SFO_OperationTPIterate(string graph, WAVE/WAVE mode, WAVE/Z/WAVE selectDataArray, WAVE/Z ignoreTPs, string opShort)
	...
	for(WAVE/Z/WAVE selectDataComp : selectDataArray)
		...
		WAVE/Z      selectData = selectDataComp[%SELECTION]
		WAVE/Z/WAVE sweepData  = SFO_OperationTPImpl(graph, mode, selectData, ignoreTPs, opShort)
		if(!WaveExists(sweepData))
			continue
		endif
...
static Function/WAVE SFO_OperationTPImpl(string graph, WAVE/WAVE mode, WAVE/Z selectDataPreFilter, WAVE/Z ignoreTPs, string opShort)
	...
	for(i = 0; i < numSelected; i += 1)
		...
		WAVE/Z/T epochMatches = SFO_FilterEpochs(epochMatchesAll, ignoreTPs)
		...
		if(!CmpStr(dataType, SF_DATATYPE_TPFIT))
				...
				if(!CmpStr(fitFunc, SF_OP_TPFIT_FUNC_EXP))
					...
					if(debugMode)
						...
						if(!CmpStr(retWhat, SF_OP_TPFIT_RET_FITQUALITY))
							...
							if(!err)
								fitResult = SFO_GetTPFitQuality(residuals, sweepData, beginTrail, endTrail)
							endif
						else
			...
			// Assemble TP data
			WAVE tpInput.data = SFO_AverageTPFromSweep(epochMatches, sweepData)
...
// tpbase()
Function/WAVE SFO_OperationTPBase(variable jsonId, string jsonPath, string graph)
...
// tpfit()
Function/WAVE SFO_OperationTPFit(variable jsonId, string jsonPath, string graph)
...
// tpinst()
Function/WAVE SFO_OperationTPInst(variable jsonId, string jsonPath, string graph)
...
// tpss()
Function/WAVE SFO_OperationTPSS(variable jsonId, string jsonPath, string graph)
...
static Function/WAVE SFO_FilterEpochs(WAVE/Z epochs, WAVE/Z ignoreTPs)
...
static Function/WAVE SFO_AverageTPFromSweep(WAVE/T epochMatches, WAVE sweepData)
	...
	tpDelta = SFO_AverageTPFromSweepImpl(tpData, tpStart, sweepData, p)
	if(numTPEpochs > 1)
...
static Function SFO_AverageTPFromSweepImpl(WAVE tpData, WAVE tpStart, WAVE sweepData, variable i)

...
/// @returns             wave with clamp state, null wave for inactive headstage
...
/// @param[in] device    Device title, e.g. "Dev1"
/// @param[in] headstage headstage number
/// @returns             wave with full clamp state, null wave for inactive headstage
...
Function FFI_TriggerAutoClampControl(string device, variable headstage, variable autoCtrl)
	...
	elseif(autoCtrl == AUTO_BRIDGEBALANCE)
		...
		FATAL_ERROR("Unknown auto clamp control: " + num2istr(autoCtrl))
	endif
End
...
Function FFI_SetClampMode(string device, variable headstage, variable clampMode)
	...
	ASSERT(ROVar(GetDataAcqRunMode(device)) == DAQ_NOT_RUNNING, "Clamp mode cannot be changed during a running acquisition.")
...
/// @param[in] device    Device title, e.g. "Dev1"
/// @param[in] headstage headstage number
/// @param[in] potential holding potential in mV

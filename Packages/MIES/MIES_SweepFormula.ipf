...
static Function/S SF_ShrinkLegend(string annotation)
	...
	for(line : lines)
		...
		if(CmpStr(opPrefixOld, opPrefix, 2) || CmpStr(suffixOld, suffix, 2))
			sweepList = CompressNumericalList(sweepList, ",")
			shrunkAnnotation += tracePrefixOld + opPrefixOld + "s " + sweepList + " " + suffixOld + "\r"
...
static Function/WAVE SF_OperationAnaFuncParam(variable jsonId, string jsonPath, string graph)
	...
	WAVE/WAVE output = SF_OperationAnaFuncParam(graph, names, allParams, SF_OP_ANAFUNCPARAM)

	return SFH_GetOutputForExecutor(output, graph, SF_OP_ANAFUNCPARAM)
End

static Function SF_OperationAnaFuncParamImpl(string graph, WAVE/Z/T names, WAVE/T allParams, variable opType)

	variable numNames, numParams, i, j, idx, allNames
	...
	for(i = 0; i < numParams; i += 1)
		...
		for(j = 0; j < numNames; j += 1)
			...
			strswitch(type)
				...
				case "wave":
				case "textwave":
					Make/FREE/T outText = {AFH_GetAnalysisParamNumerical(name, params)}
					WAVE out = outText
					break
				case "":
					// no such parameter
					continue
				default:
					ASSERT(0, "Invalid parameter type: " + type)
			endswitch

			Note/K out, note(paramsSingle)

			[STRUCT RGBColor color] = GetTraceColor(NUM_RESERVED_COLORS - j)
			JWN_SetWaveInWaveNote(out, SF_META_TRACECOLOR, {color.red, color.green, color.blue})
...
static Function/WAVE SF_OperationLabnotebook(variable jsonId, string jsonPath, string graph)
	...
	WAVE/T lbnKeys = SFH_GetArgumentAsWave(jsonID, jsonPath, graph, SF_OP_LABNOTEBOOK, 0, singleResult = 1)
	SFH_ASSERT(IsTextWave(lbnKeys) , "First parameter needs to be a text array of labnotebook keys.")
...
static Function/WAVE SF_OperationLabnotebookImpl(string graph, WAVE/T allLBNKeys, WAVE/Z selectData, variable mode, string opShort)
	...
	for(lbnKey : allLBNKeys)
		for(i = 0; i < numSelected; i += 1)
			...
			[STRUCT RGBColor color] = GetTraceColor(NUM_RESERVED_COLORS - lbnIndex)
			JWN_SetWaveInWaveNote(out, SF_META_TRACECOLOR, {color.red, color.green, color.blue})

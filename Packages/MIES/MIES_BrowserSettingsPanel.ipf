...
Function BSP_AddTracesForEpochs(string win)
	...
	variable sweepNumber, traceIndex, channelType, channelNumber, mapIndex, isSweepBrowser
	STRUCT RGBColor c
	string xaxis, yaxis, axes, axis, levels_x_name, levels_y_name, name, idPart, level_x_trace

	isSweepBrowser = BSP_IsSweepBrowser(win)
	if(!BSP_IsDataBrowser(win) && !isSweepBrowser)
	...
	for(i = 0; i < numTraces; i += 1)
		...
		if(isSweepBrowser)
			...
			DFREF  sweepDFR = SB_GetSweepDF(win, mapIndex)
			[WAVE numericalValues, WAVE textualValues] = SB_GetLabNotebooks(sweepMap, mapIndex)
		else
			WAVE/Z numericalValues = BSP_GetLogbookWave(win, LBT_LABNOTEBOOK, LBN_NUMERICAL_VALUES, sweepNumber = sweepNumber)
			WAVE/Z textualValues   = BSP_GetLogbookWave(win, LBT_LABNOTEBOOK, LBN_TEXTUAL_VALUES, sweepNumber = sweepNumber)
			DFREF  sweepDFR        = DB_GetSweepDF(win, sweepNumber)
		endif

...
static Function AutoTP_NeedsAdjustmentValues_IGNORE(string device)
	...
	// HS0: out+, good, out+, good, good, good, ...
	overrideResults[][0][%Factor]  = 0.26 // good
	overrideResults[0][0][%Factor] = 0.29 // out+
	overrideResults[2][0][%Factor] = 0.29 // out+

	//      good, out+, good, out+, good, good, good, ...
	overrideResults[][0][%Voltage]  = 0.016 // mV
	overrideResults[1][0][%Voltage] = 0.025 // out+
	overrideResults[3][0][%Voltage] = 0.025 // out+

	overrideResults[][0][%BaselineFitResult] = TP_BASELINE_FIT_RESULT_OK

	// HS1: out-, good, out-, good, good, good, ...
	overrideResults[][1][%Factor]  = 0.26 // good
	overrideResults[0][1][%Factor] = 0.22 // out-
	overrideResults[2][1][%Factor] = 0.22 // out-

	//      good, out-, good, out-, good, good, good, ...
	overrideResults[][1][%Voltage]  = 0.016 // good
	overrideResults[1][1][%Voltage] = 0.005 // out-
	overrideResults[3][1][%Voltage] = 0.005 // out-

	// amplitude tests are shifted by one as we always take a baseline and amplitude value when checking either
...
// UTF_TD_GENERATOR HardwareMain#DeviceNameGeneratorMD1
static Function AutoTP_NeedsAdjustmentValues([string str])
	STRUCT DAQSettings s
	InitDAQSettingsFromString(s, "MD1_RA1_I0_L0_BKG_1")

	BasicHardwareTests#AcquireData(s, str, startTPInstead = 1, preAcquireFunc = AutoTP_NeedsAdjustmentValues_IGNORE)
End

static Function AutoTP_NeedsAdjustmentValues_REENTRY([string str])
	...
	// logic for checking
	// get wave sizes, prefill with pass and fill in the failures
	...
	Duplicate/FREE entries[%baselinePass_HS0], blPassRef_HS0
	blPassRef_HS0[]  = 1
	blPassRef_HS0[0] = 0
	blPassRef_HS0[2] = 0

	Duplicate/FREE entries[%baselinePass_HS1], blPassRef_HS1
	blPassRef_HS1[]  = 1
	blPassRef_HS1[0] = 0
	blPassRef_HS1[2] = 0
	...
	CHECK_EQUAL_WAVES(entries[%baselinePass_HS0], blPassRef_HS0)
	CHECK_EQUAL_WAVES(entries[%baselinePass_HS1], blPassRef_HS1)

	/// @todo this should check for {1, 1, NaN, ...} but GetLastSettings returns the latest setting on any headstage
	/// see https://github.com/AllenInstitute/MIES/issues/1170
	CHECK_EQUAL_WAVES(entries[%autoTPQC], {1, NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN}, mode = WAVE_DATA)

	CHECK(IsConstant(entries[%baselineFitResult_HS0], TP_BASELINE_FIT_RESULT_OK))
	CHECK(IsConstant(entries[%baselineFitResult_HS1], TP_BASELINE_FIT_RESULT_OK))

	CHECK_EQUAL_WAVES(entries[%autoTPEnable], {0, 0, NaN, NaN, NaN, NaN, NaN, NaN, NaN}, mode = WAVE_DATA)
	CHECK_EQUAL_WAVES(entries[%amplitudeIC], {21.92, 122.04, NaN, NaN, NaN, NaN, NaN, NaN, NaN}, mode = WAVE_DATA, tol = 1e-1)
	CHECK_EQUAL_WAVES(entries[%baselineFrac], {NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN, 0.321800}, mode = WAVE_DATA, tol = 1e-6)
End

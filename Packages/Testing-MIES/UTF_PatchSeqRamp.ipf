...
static Function PS_RA3_REENTRY([str])
	...
	WAVE/Z durations = GetPulseDurations_IGNORE(sweepNo, str)
	CHECK_GT_VAR(durations[0], SPIKE_POSITION_MS - PSQ_RA_BL_EVAL_RANGE)
	CHECK(durations[0] < SPIKE_POSITION_TEST_DELAY_MS)
	CHECK_GT_VAR(durations[1], SPIKE_POSITION_MS - PSQ_RA_BL_EVAL_RANGE)
	CHECK(durations[1] < SPIKE_POSITION_TEST_DELAY_MS)
	CHECK_GT_VAR(durations[2], SPIKE_POSITION_MS - PSQ_RA_BL_EVAL_RANGE)
	CHECK(durations[2] < SPIKE_POSITION_TEST_DELAY_MS)

...
- Operations accepting a `seltag` argument (e.g. `ivscc_apfrequency()`)
  auto-group by existing experiment tags when no explicit `seltag` is given.
...
building blocks, via this pattern:

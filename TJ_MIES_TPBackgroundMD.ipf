...
Function ITC_BkrdTPFuncMD(s)
	...
	do // works through list of active devices
		...
		if(ITCFIFOAvailAllConfigWave[ADChannelToMonitor][2] >= (GlobalTPDurationVariable * ActiveDeviceList[i][4])) // chops up 100 pulse TP wave into single pulses based on location in data acquisition
			if(ActiveDeviceList[i][4] >= 100)
				ActiveDeviceList[i][4] = 1
			endif
			startPoint = ((GlobalTPDurationVariable * 2) * ActiveDeviceList[i][4])
			DM_CreateScaleTPHoldWaveChunk(panelTitle, startPoint, GlobalTPDurationVariable)
		...
		if(ITCFIFOAvailAllConfigWave[ADChannelToMonitor][2] >= (StopCollectionPoint * ActiveDeviceList[i][3])) // advances the FIFO is the TP sweep has reached it's end
			// functions below work with TP wave - ideally they should occur before the FIFO advance, not sure if they can "fit" there
			//DM_CreateScaleTPHoldingWave(panelTitle)
			//startPoint = (GlobalTPDurationVariable * ActiveDeviceList[i][3])
			//DM_CreateScaleTPHoldWaveChunk(panelTitle,startPoint,GlobalTPDurationVariable)

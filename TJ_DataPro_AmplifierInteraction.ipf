...
wave ChanAmpAssign
	...
	ControlInfo popup_Settings_Amplifier
		if(v_value>1)
...
Function UpdateChanAmpAssignPanel()
Variable HeadStageNo
wave ChanAmpAssign
...
Popupmenu Popup_Settings_VC_DA mode=(ChanAmpAssign[0][HeadStageNo]+1)
Setvariable setvar_Settings_VC_DAgain value=_num:ChanAmpAssign[1][HeadStageNo]
Popupmenu Popup_Settings_VC_AD mode=(ChanAmpAssign[2][HeadStageNo]+1)
Setvariable setvar_Settings_VC_ADgain_0 value=_num:ChanAmpAssign[3][HeadStageNo]

Popupmenu Popup_Settings_IC_DA mode=(ChanAmpAssign[4][HeadStageNo]+1)
Setvariable setvar_Settings_IC_DAgain value=_num:ChanAmpAssign[5][HeadStageNo]
Popupmenu  Popup_Settings_IC_AD mode=(ChanAmpAssign[6][HeadStageNo]+1)
Setvariable setvar_Settings_IC_ADgain value=_num:ChanAmpAssign[7][HeadStageNo]

Popupmenu popup_Settings_Amplifier mode=ChanAmpAssign[10][HeadStageNo]
...
Function PopMenuProc(ctrlName,popNum,popStr) : PopupMenuControl
	...
	Variable popNum
	String popStr
UpdateChanAmpAssignStorageWave()
End
...
Function ApplyClampModeSavedSettings(HeadStageNo, ClampMode)
...
If(ClampMode==1)
	...
	SetVariable $DAGain value=_num:ChanAmpAssign[5][HeadStageNo]
	...
	SetVariable $ADGain value=_num:ChanAmpAssign[7][HeadStageNo]
	else
...
Function RemoveClampModeSettings(HeadStageNo, ClampMode)
variable HeadStageNo, ClampMode// 0 = VC, 1 = IC
wave ChanAmpAssign
string DACheck, DAGain, ADCheck, ADGain
If(ClampMode==0)
...
Function CheckProc_ClampMode(ctrlName,checked) : CheckBoxControl
	...
	string HeadStageCheckBox = "Check_DataAcq_Cell"
	...
	if(mod(RadioButtonNo,2)==0)//even numbers
		...
		controlinfo $HeadStageCheckBox
		
		if(v_value==1)//checks to see if headstage is "ON"
		ApplyClampModeSavedSettings((RadioButtonNo/2), 0)//Applies VC settings for headstage
		RemoveClampModeSettings((RadioButtonNo/2), 1)
		endif
		...
		if(v_value==1)//checks to see if headstage is "ON"
		ApplyClampModeSavedSettings(((RadioButtonNo-1)/2), 1)//Applies IC settings for headstage
		RemoveClampModeSettings(((RadioButtonNo-1)/2), 0)
		endif
...
Function CheckProc_HeadstageCheck(ctrlName,checked) : CheckBoxControl
	...
	ControlInfo $RadioButtonName
	ClampMode=v_value
	
	If(Checked==0)
	RemoveClampModeSettings(HeadStageNo, ClampMode)
	else
	ApplyClampModeSavedSettings(HeadStageNo, ClampMode)
	endif

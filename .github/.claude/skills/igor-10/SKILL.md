...
PythonFile "myScript.py"

// Run Python code and receive results back into an Igor wave
Make/FREE arrayFromPython
Python execute = "arr : ndarray = waveAnalysis()", array = {"arr", arrayFromPython}
```

### Calling Igor from Python (igorpro module)

```python
import igorpro
import numpy as np

# Access a data folder and its waves
dataset = igorpro.folder('root:analysis').waves()
for wave in dataset:
    results = runCalculations(wave.asarray())  # converts wave to numpy array

# Set a wave value from Python
igorpro.wave('root:myWave')[3] = 42.0
```

### Calling Igor built-in functions from Python (new in 10.01 — igorpro.fn submodule)

```python
import igorpro

# Call any Igor built-in, external, or user-defined function directly
sorted_list = igorpro.fn.sortlist('x;c;e;g;d;a', ';', 1)
result = igorpro.fn.mean(igorpro.wave('root:myWave'))
```

`igorpro.fn` supports returning: real numbers, complex numbers, strings,
wave references, and data folder references.

### Python Console
Available via Python → Open Console. Allows line-by-line Python execution
with access to the Igor environment.

### Python version support
Igor 10.00 supports Python 3.x. Igor 10.01 adds support for Python 3.14
(standard installation only, not free-threaded).
